...
def azure_stop_schedule(endpoint_id):
    """Stop the schedule for an Azure connector endpoint.

    Body JSON: { "user_id": ..., "execution_key": "azure_endpoint:<id>:<user>:interval" }
    Mirrors apiConnector.stop_schedule but targets azure_external_app_endpoints
    and AzureAPIConnectorScheduler.
    """
    try:
        body = request.get_json(force=True) or {}
        user_id = body.get("user_id") or _extract_user_id()
        ...
        for sch in schedules:
            if sch.get("execution_key") == execution_key:
                ...
                if sch.get("celery_entry"):
                    AzureAPIConnectorScheduler.disable_celery_entry(sch["celery_entry"])
                if sch.get("celery_task_id"):
                    AzureAPIConnectorScheduler.revoke_task(sch["celery_task_id"])
                if sch.get("celery_task_ids"):
                    for tid in sch["celery_task_ids"]:
                        AzureAPIConnectorScheduler.revoke_task(tid)
                break

        _save_azure_endpoint_schedule(
            endpoint_id, schedules[0] if len(schedules) == 1 else schedules
        )
        asyncio.run(
            AzureAPIConnectorScheduler.make_schedule_disabled(stop_key=execution_key)
        )
