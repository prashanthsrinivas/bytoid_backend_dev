...
from utils.key_rotation_manager import SecureKMSService as _GcpRunKMSService
_gcp_run_kms = _GcpRunKMSService()


def _enc_run(user_id, v):
    s = v if isinstance(v, str) else json.dumps(v, default=str)
    enc = _gcp_run_kms.encrypt(user_id, s)
...
def _dec_run(user_id, v):
    if isinstance(v, dict) and "encrypted_key" in v:
        raw = _gcp_run_kms.decrypt(user_id, v["encrypted_key"], v["iv"], v["ciphertext"])
        try:
