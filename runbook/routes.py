...
def get_runbook_results(runbook_id):
    ...
    try:
        ...
        # Shared access fallback: run when user has no owned results for this
        # runbook. Covers both cases — user doesn't own the runbook at all, and
        # user owns the runbook template (e.g. via SU mode) but the results
        # were generated by another admin and shared back. Wrapped so any
        # malformed shared_reports.json shape can't 500 the whole endpoint.
        if not results:
            try:
