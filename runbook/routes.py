...
def get_runbook_results(runbook_id):
    ...
    try:
        ...
        # Always merge shared results for this runbook — not just when owned
        # results are absent. A user may own the runbook (created via SU mode)
        # but have results that were generated by another admin and shared back.
        shared_reports = get_user_shared_reports(user_id)
        ...
        if allowed_result_ids:
            owned_result_ids = {r.get("result_id") for r in (results or [])}
            ...
            try:
                ...
                for rid in allowed_result_ids:
                    if rid in owned_result_ids:
                        continue
                    main_user_id = shared_reports[rid].get("mainuser_id")
                ...
                results = (results or []) + shared_results
