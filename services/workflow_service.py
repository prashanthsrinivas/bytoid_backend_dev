...
class WorkflowRunnerV2:
    ...
    def _trigger_runbook_owner(self, runbook_id: str) -> None:
        ...
        For assigned questionnaire clones: resolves admin_id (from the
        workflow JSON's assignment_id/assignment_admin_id, or by DB lookup
        against intake_workflow_assignments using the workflow base), merges
        the respondent's Q&A into the admin's original workflow in S3, then
        fires the task under admin_id with the admin's reference filename.
        """
        from utils.celery_base import create_playbook_runbook_task

        admin_id = self.workflow_json.get("assignment_admin_id")
        assignment_id = self.workflow_json.get("assignment_id")
        reference_filename = self.workflow_json.get("reference_filename")

        # Resolve admin_id from DB if missing (covers clones created before
        # assignment context was propagated and clones with reference_filename
        # but no admin id).
        if not admin_id:
            conn = connect_to_rds()
            try:
                with conn.cursor() as cur:
                    if assignment_id:
                        cur.execute(
                            "SELECT admin_id, workflow_filename "
                            "FROM intake_workflow_assignments "
                            "WHERE assignment_id = %s LIMIT 1",
                            (assignment_id,),
                        )
                    else:
                        # Discover by recipient + workflow base prefix
                        wf_base = base_name(self.filename)
                        cur.execute(
                            "SELECT admin_id, workflow_filename "
                            "FROM intake_workflow_assignments "
                            "WHERE recipient_user_id = %s "
                            "AND (workflow_filename = %s OR workflow_filename = %s "
                            "OR workflow_filename LIKE %s) "
                            "ORDER BY assignment_id DESC LIMIT 1",
                            (
                                self.userid,
                                reference_filename or self.filename,
                                (reference_filename or self.filename or "").rstrip(".json"),
                                f"{wf_base}%",
                            ),
                        )
                    row = cur.fetchone()
        ...
        if admin_id and reference_filename:
