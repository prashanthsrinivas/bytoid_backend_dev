...
def get_entity(text_input, user_id, cursor):
    ...
    if match:
        ...
        if parts:
            like_conditions = " OR ".join(["LOWER(SUBSTRING_INDEX(email_id, '@', 1)) LIKE LOWER(%s)"] * len(parts))
        ...
        for token in doc:
            if token.pos_ == "PROPN":
                ...
                if temp_name:
                    ...
                    temp_name = []
        if temp_name:
            person_names.append((" ".join(temp_name), start_idx, token.idx + len(temp_name[-1])))
...
def get_search_summary(text_input, anonymised_input, entity_groups):
    ...
    try:
        ...
        message_payload = json.dumps(text_input, indent=2)
        full_prompt = update_prompt_template.replace(
                "{emails_list}", message_payload
            )
        full_prompt = full_prompt.replace(
                "{user_query}", anonymised_input
            )
        entity_groups_payload = json.dumps(entity_groups, indent=2)
        full_prompt = full_prompt.replace(
                "{entity_groups}", entity_groups_payload
            )
            ...
            if 'message_id' in db_query:
                    ...
                    for row in db_results:
                        ...
                        if input_data:
                            if isinstance(input_data, list):
                                message = [msg for msg in input_data if msg["id"] == message_id]
                                if message:
                ...
                if  db_results:
                    ...
                    for row in db_results:
                        ...
                        sql_query = "select content_ref from messages where conversation_id_fk = %s"
...
def anonymize_before_summarisation(message_body, user_query):
    ...
    def filter_overlaps(results):
        filtered = []
        for res in sorted(results, key=lambda r: (r.start, -(r.end - r.start))):  # Ensures that for overlapping spans, the longest one comes first.
            if not any(res.start >= f.start and res.end <= f.end for f in filtered): # This keeps an entity only if it is not fully inside another already chosen one.
                filtered.append(res)
        return filtered
...
def generate_entity_groups(pii_mapping):
    ...
    emails = {t: v.lower() for t, v in pii_mapping.items() if t.startswith("<EMAIL_ADDRESS_")}
    ...
    for token, value in pii_mapping.items():
        ...
        if token.startswith("<PERSON_"):
            ...
            for other_token, other_value in persons.items():
                if other_token not in processed_tokens and names_similar(person_name, other_value):
        ...
        # Compare EMAIL tokens
            ...
            for person_token, person_value in persons.items():
                if person_token not in processed_tokens and names_similar(email_name, person_value):
            ...
            for other_email_token, other_email_value in emails.items():
                if other_email_token not in processed_tokens and other_email_value == value_lower:
...
def search_emails():
    try:
        ...
        if entities: 
            ...
            spans = merge_spans(spans) # spans contain unique start and end index of enitities
        ...
        if name_resolved:
            folder_names = [item["id"] for item in entities]   # folder_names contain client_id
            text_input, name_mapping = anonymizer(text_input, entities)  # the names of entities are replaced by secret_name_1 ....

        text_input = normalize_query(text_input)
        ...
        # psemantic search in the lance table
        client = UmailLanceClient(user_id)
        if semantic_query and semantic_query !="null":
            lance_mails = client.search_email_from_lance(folder_names, user_id, semantic_query )
        ...
        if db_queries and db_queries != "null":
            if semantic_query and semantic_query != "null":
                if db_mails and lance_mails:
                    ...
                    lance_conversation_ids = {mail['conversation_id'] for mail in lance_mails}
                    common_conversation_ids = db_conversation_ids.intersection(lance_conversation_ids)
                    messages_to_process = [mail for mail in db_mails + lance_mails 
        ...
        anonymised_text, anonymised_input, pii_mapping = anonymize_before_summarisation(message_body, user_query )
